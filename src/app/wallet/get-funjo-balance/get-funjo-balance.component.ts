...
@Component({
  selector: 'app-get-funjo-balance',
  templateUrl: './get-funjo-balance.component.html',
  styleUrls: ['./get-funjo-balance.component.css']
})
export class GetFunjoBalanceComponent implements OnInit {
    ...
    if(transno != ''){
      this._api.post('/secure/updatetransactionnumber', obj).subscribe(data => {
        if (data.status == 1) {
          ...
          this.fiurthSection = false;
          // this.addfund.reset();
          this.router.navigateByUrl('/deposit-summary');
